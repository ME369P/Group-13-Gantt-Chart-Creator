    ...
    def draw(self):
        ...
        if len(self.TaskNames) > 0:
            ...
            for item in self.TaskNames:
                self.axes.broken_barh([(float(self.TaskStarts[listindex]), 
                float(self.TaskEnds[listindex]))], (graphpositionindex*10, 10), facecolors =self.TaskColors[listindex])
...
class myFrame(wx.Frame):
    def __init__(self):
        ...
        self.Bind(wx.EVT_MENU,self.Importcsv,fileItem1)
        #self.Bind(wx.EVT_MENU,self.Savecsv,fileItem2)
        self.toppanel.draw()
...
# Code for method to allow user to pick a file from directory
...
'''
# Code for method to allow user to save a file to directory
    def Savecsv(self,e):
        dlg = wx.FileDialog(
